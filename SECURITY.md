...
4. **Rate limiting** — `POST /admin` and `POST /ui/login` only; only **failed** auth attempts count per IP (defaults: **2** per 60s); once blocked, **429** until the window expires, even with a valid token; successful auth is not counted; set `RATE_LIMIT_MAX_ATTEMPTS=0` to disable. Behind Traefik, set `TRUSTED_PROXIES` so limits use the real client IP.
