...
4. **Rate limiting** — `POST /admin` and `POST /ui/login` return **429** after too many attempts per client IP (defaults: 30 per 60s; set `RATE_LIMIT_MAX_ATTEMPTS=0` to disable).
